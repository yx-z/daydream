...
constexpr static inline auto is_monadic_v =
    is_instance<T, Either>::value || is_instance<T, Maybe>::value ||
...
struct Either {
  ...
  constexpr Either(const Either<L2, R2>& e)
      : _left{e.left()}, _right{e.right()} {}

  template <typename L2, typename R2>
  constexpr Either(Either<L2, R2>&& e)
      : _left{std::move(e.left())}, _right{std::move(e.right())} {}
...
struct Maybe {
  ...
  template <
      typename Func,
      std::enable_if_t<std::is_convertible_v<std::invoke_result_t<Func>, T>,
                       int> = 0>
...
constexpr static auto resR =
    Either<int, float>{nullptr, 10.0} | eitherOperations;
