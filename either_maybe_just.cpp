...
template <typename T>
constexpr static inline auto is_monadic_v = is_instance<T, Either>::value || is_instance<T, Maybe>::value || is_instance<T, Just>::value;
...
template <typename L, typename R>
struct Either {
    constexpr Either(L l, std::nullptr_t)
        : _left{ std::move(l) }
    {
    }

    constexpr Either(std::nullptr_t, R r)
        : _right{ std::move(r) }
    {
    }

    template <typename L2, typename R2>
    constexpr Either(const Either<L2, R2>& e)
        : _left{ e.left() }
        , _right{ e.right() }
    {
    }
    ...
    template <typename L2, typename R2>
    constexpr Either(Either<L2, R2>&& e)
        : _left{ std::move(e.left()) }
        , _right{ std::move(e.right()) }
    {
    }
    ...
    constexpr L left_or(const L& l) const { return has_left() ? left() : l; }
    ...
    constexpr R right_or(const R& r) const { return has_right() ? right() : r; }
...
struct Maybe {
    ...
    constexpr Maybe(std::optional<T> value)
        : _val{ std::move(value) }
    {
    }
    ...
    constexpr T value_or(const T& t) const { return has_value() ? value() : t; }

    constexpr T operator||(const T& t) const { return value_or(t); }
    ...
    template <typename Func,
        std::enable_if_t<std::is_convertible_v<std::invoke_result_t<Func>, T>, int> = 0>
...
struct Just {
    ...
    constexpr auto operator|(Func&& cont) const
    {
        auto res = cont(value());
        if
            constexpr(is_monadic_v<decltype(res)>) { return res; }
        else {
            return Just{ res };
        }
    }
...
constexpr static auto basicUsage = Just{ 12 } |
    ...
    [](const auto i) { return i; }, [](const auto f) { return f + 2.0; }
...
// chain then drop
constexpr static auto dropped = Either<int, float>{ 12, nullptr } | DropRight{} |
    [](const auto i) { return i + 1; };
static_assert((dropped || 12) == 13);

// can chain Continues first, then apply to different input
constexpr static auto justOperations = Continue{ [](const auto i) { return i + 1; } } | Continue{ [](const auto i) { return i + 2; } };
...
constexpr static auto eitherOperations = Continue{ [](const auto i) { return i + 1; },
